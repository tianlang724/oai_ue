...
int trx_eth_read_udp(openair0_device *device, openair0_timestamp *timestamp, void **buff, int nsamps, int cc) {
	...
	if(state<=0)
	{
		...
		{		
			...
			printf("[nerlint_read] read %s\n",(char *)NLMSG_DATA(nlh_rx));
		}
		else
		{
...
int trx_eth_write_udp(openair0_device *device, openair0_timestamp timestamp, void **buff, int nsamps,int cc, int flags) {	
  printf("[trx_eth_write_udp] zh start\n");
  int bytes_sent=0;
  eth_state_t *eth = (eth_state_t*)device->priv;
  int Mod_id = device->Mod_id;
  int sendto_flag =0;
...
#endif
      /* Send packet */
      printf("[eth_write_func]zh addr=%s,port=%d\n",inet_ntoa(dest_addr[Mod_id].sin_addr),ntohs(dest_addr[Mod_id].sin_port));
	 // printf("[eth_write_func] zh byte=%d,send ip=%s,port=%d\n",bytes_sent,inet_ntoa(dest_addr[Mod_id].sin_addr,ntohs(dest[Mod_id].sin_port)));
...
int trx_eth_read_udp(openair0_device *device, openair0_timestamp *timestamp, void **buff, int nsamps, int cc) {
  printf("[trx_eth_read_udp] zh start\n"); 
...
# if 0
  ...
  void 	      *msg;
  ssize_t      msg_len;
  printf("[eth_set_dev_conf_udp]zh start\n");
  ...
  printf("[eth_set_dev_conf_udp]zh addr=%s,port=%d\n",inet_ntoa(dest_addr[Mod_id].sin_addr),ntohs(dest_addr[Mod_id].sin_port));
