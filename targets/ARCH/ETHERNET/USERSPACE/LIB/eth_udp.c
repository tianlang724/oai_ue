...
#include <linux/netlink.h>   //zh xiugai
...
#define	MAX_PAYLOAD 1200//maximum payload size zh change from 1024
char nl_rx_buf[MAX_PAYLOAD];
int sock_fd;
struct sockaddr_nl src_addr,nl_dest_addr;
struct nlmsghdr *nlh_tx = NULL;
struct nlmsghdr *nlh_rx = NULL;
struct iovec iov_tx;
struct iovec iov_rx={nl_rx_buf,sizeof(nl_rx_buf)};
struct msghdr msg_tx;
struct msghdr msg_rx;
int eth_socket_init_udp(openair0_device *device) {
//Create a socket
	printf("[netlink_init]zh begin init\n");
	fflush(stdout);
	...
	src_addr.nl_groups=0;//multi cas
	bind(sock_fd,(struct sockaddr*)&src_addr, sizeof(src_addr));
	//nl_dest_address
	memset(&nl_dest_addr,0,sizeof(nl_dest_addr));
	nl_dest_addr.nl_family=AF_NETLINK;
	nl_dest_addr.nl_pid=0;/* For Linux Kernel */
	nl_dest_addr.nl_groups=0;/* unicast */
	
	//rx init
	nlh_rx=(struct nlmsghdr *)malloc(NLMSG_SPACE(MAX_PAYLOAD));

	//tx init
	nlh_tx=(struct nlmsghdr *)malloc(NLMSG_SPACE(MAX_PAYLOAD));

	//20171117 set recv timeout
	struct timeval tv_out;
	tv_out.tv_sec=0;
	tv_out.tv_usec=100;
	setsockopt(sock_fd, SOL_SOCKET, SO_RCVTIMEO, &tv_out, sizeof(tv_out));
	printf("[netlink_init] zh init end\n");
	fflush(stdout);
	return 0;
}
int trx_eth_write_udp(openair0_device *device, openair0_timestamp timestamp, void **buff, int nsamps,int cc, int flags) {
	printf("[trx_eth_write_udp]zh netlink enter\n");
	int state_smg_tx;
	memset(nlh_tx,0,sizeof(struct nlmsghdr));
	nlh_tx->nlmsg_len=NLMSG_SPACE(MAX_PAYLOAD);
	nlh_tx->nlmsg_type=20;
	nlh_tx->nlmsg_seq=0;
	...
	msg_tx.msg_name=(void *)&nl_dest_addr;
	msg_tx.msg_namelen=sizeof(nl_dest_addr);
	msg_tx.msg_iov=&iov_tx;
	msg_tx.msg_iovlen=1;
	state_smg_tx=sendmsg(sock_fd,&msg_tx,0);
	if(state_smg_tx==-1)
	{
		printf("get error sendmsg = %s\n",strerror(errno));
		return 0;
	}
	printf("[trx_eth_write_udp]zh netlink send %d\n",state_smg_tx);
	return state_smg_tx;
}
int trx_eth_read_udp(openair0_device *device, openair0_timestamp *timestamp, void **buff, int nsamps, int cc) {
	
	memset(nlh_rx,0,sizeof(struct nlmsghdr));
	nlh_rx->nlmsg_len=NLMSG_SPACE(MAX_PAYLOAD);
	nlh_rx->nlmsg_type=0;
	nlh_rx->nlmsg_seq=0;
	nlh_rx->nlmsg_pid=100;/*self pid*/
	nlh_rx->nlmsg_flags=0;
	iov_rx.iov_base=(void*)nlh_rx;
	iov_rx.iov_len=nlh_rx->nlmsg_len;
	memset(&msg_rx,0,sizeof(msg_rx));
	msg_rx.msg_name=(void *)&nl_dest_addr;
	msg_rx.msg_namelen=sizeof(nl_dest_addr);
	msg_rx.msg_iov=&iov_rx;
	msg_rx.msg_iovlen=1;

	int state;
	state = recvmsg(sock_fd,&msg_rx,0);
	if(state<=0)
	{
			printf("[netlink_read]state=%d,errno=%d\n",state,errno);
			return 0;
	}else{	
		if(NLMSG_DATA(nlh_rx))	
		{		
			memcpy(buff[0],NLMSG_DATA(nlh_rx),nlh_rx->nlmsg_len);
			printf("[netlink_read] read state=%d,datalen=%d\n",state,nlh_rx->nlmsg_len);
			fflush(stdout);
		}
		else
		{
			printf("[netlink_read]nothing\n");
		}
		return state;
	}
