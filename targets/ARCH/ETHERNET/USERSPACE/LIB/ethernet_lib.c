...
int trx_eth_start(openair0_device *device) {
  printf("[trx_eth_start]zh start\n");
  fflush(stdout);
  printf("Setting ETHERNET to UDP__MODE\n");
...
#endif
  printf("[trx_eth_start]zh will return 0\n");
  fflush(stdout);
  return 0;
}
...
int trx_eth_set_freq(openair0_device* device, openair0_config_t *openair0_cfg,int exmimo_dump_config) {
    printf("[trx_eth_set_freq] do nothing\n");
	fflush(stdout);
	return(0);
}
...
#endif
  printf("[ethernet_tune] zh do nothing\n");
  fflush(stdout);  
  return 0;
}
...
int transport_init(openair0_device *device, openair0_config_t *openair0_cfg, eth_params_t * eth_params ) {
  printf("[transport_init]zh enter\n");
  fflush(stdout);
