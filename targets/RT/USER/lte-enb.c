...
void tx_fh_if5(PHY_VARS_eNB *eNB,eNB_rxtx_proc_t *proc) {
  ...
  eNB->ifdevice.trx_write_func(&eNB->ifdevice,null,eNB->common_vars.txdata[0],siglen,fp->nb_antennas_tx,flags);
}
...
void rx_fh_if5(PHY_VARS_eNB *eNB,int *frame, int *subframe) {
  printf("[rx_fh_if5]zh enter\n");
  fflush(stdout);

  LTE_DL_FRAME_PARMS *fp = &eNB->frame_parms;
  int siglen=fp->samples_per_tti,flags=1;
  eNB->ifdevice.trx_read_func(&eNB->ifdevice,null,eNB->common_vars.rxdata[0],siglen,fp->nb_antennas_tx);
...
static void* eNB_thread_single( void* param ) {
  ...
  // zh add 20170925
  printf("[eNB_thread_single] zh send cond_asynch_rxtx\n");
  fflush(stdout);
  ...
  // This is a forever while loop, it loops over subframes which are scheduled by incoming samples from HW devices
  while (!oai_exit) {
    printf("[eNB_thread_single] zh wait in loop\n");
	fflush(stdout);
    // these are local subframe/frame counters to check that we are in synch with the fronthaul timing.
    // They are set on the first rx/tx in the underly FH routines.
    if (subframe==9) { 
    ...
    // synchronization on FH interface, acquire signals/data and block
	// zh add if(frame>100) 20170915
	if(frame>100){
    if (eNB->rx_fh) eNB->rx_fh(eNB,&frame,&subframe);
    else AssertFatal(1==0, "No fronthaul interface : eNB->node_function %d",eNB->node_function);
	}else{
		printf("[eNB_thread_single]zh pass readfunc\n");
		fflush(stdout);
	}
