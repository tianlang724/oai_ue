...
static void *UE_thread_synch(void *arg) {
    ...
    printf("[UE_thread_synch] zh enter\n");
	fflush(stdout);
    if (UE->UE_scan == 0) {
        int ind;
        for ( ind=0;
    ...
    while (oai_exit==0) {
		...
		printf("[UE_thread_synch] get signal\n");
		fflush(stdout);
        switch (sync_mode) {
        case pss:
        ...
        case pbch:
            ...
            if (initial_sync( UE, UE->mode ) == 0) {
                ...
                LOG_I( HW, "Got synch: hw_slot_offset %d, carrier off %d Hz, rxgain %d (DL %u, UL %u), UE_scan_carrier %d\n",
					...
					printf("[UE_thread_synch]zh test0\n");
					fflush(stdout);
                    sleep(1);
					printf("[UE_thread_synch]zh test1\n");
					fflush(stdout);
                    init_frame_parms(&UE->frame_parms,1);
					printf("[UE_thread_synch]zh test2\n");
					fflush(stdout);
		...
		if (UE->UE_scan_carrier == 1) {
		printf("[UE_thread_synch]zh UE_scan_carrier=1 and set to 0\n");
		fflush(stdout);
		  UE->UE_scan_carrier = 0;
                } else {
					printf("[UE_thread_synch]zh UE_scan_carrier=0\n");
					fflush(stdout);
                    AssertFatal ( 0== pthread_mutex_lock(&UE->proc.mutex_synch), "");
				...
				printf("[initial_synch] initial synch failed\n");
				fflush(stdout);
				// zh modidy 20171119
				/*
                if (UE->UE_scan_carrier == 1) {
...
static void *UE_thread_rxn_txnp4(void *arg) {
    ...
    while (!oai_exit) {
		printf("[UE_thread_rxn_txnp4] wait in loop\n");
		fflush(stdout);
		...
		printf("[UE_thread_rxn_txnp4] rx_frame=%d,tx_frame=%d,rx_subframe=%d,tx_subframe=%d\n",proc->frame_rx,proc->frame_tx
						,proc->subframe_rx,proc->subframe_tx);
		fflush(stdout);
        ...
        if ((sf_type == SF_DL) ||
			...
			printf("[UE_thread_rxn_txnp4] will call phy_procedures_UE_RX\n");
			fflush(stdout);
            phy_procedures_UE_RX_zh( UE, proc, 0, 0, UE->mode, no_relay, NULL );
        }
...
void *UE_thread(void *arg) {
 printf("[UE_thread] zh enter\n");
    ...
    if (oaisim_flag == 0){
	   ...
	   // zh xiugai 20170926 修改为使用transport
	   int ret=openair0_transport_load(&(UE->rfdevice),&openair0_cfg[0],"");
	   if(ret<0){
			   printf("Exiting ,cannot initialize transport protocal\n");
			   exit(-1);
	   }
	}
    ...
    AssertFatal(UE->rfdevice.trx_start_func(&UE->rfdevice) == 0, "Could not start the device\n");
    while (!oai_exit) {
		//memset(UE->common_vars.rx_buff[0],0,sizeof(int32_t)*1500);
		printf("[UE_thread]zh in loop\n");
		fflush(stdout);
        AssertFatal ( 0== pthread_mutex_lock(&UE->proc.mutex_synch), "");
        ...
        if (is_synchronized == 0) {
			printf("[UE_thread] is_synchronized=0\n");
			fflush(stdout);
            if (instance_cnt_synch < 0) {  // we can invoke the synch
                // grab 10 ms of signal and wakeup synch thread
				printf("[UE_thread]zh in loop synchronized=0,instance_cnt_synch<0\n");
				fflush(stdout);
                if (UE->mode != loop_through_memory){
                ...
                if (instance_cnt_synch == 0) {
					...
					printf("[UE_thread]send cond_synch\n");
                } else {
		...
		AssertFatal ( 0== pthread_mutex_unlock(&UE->proc.mutex_synch), "");
						...
						printf("[UE_thread] in synch read nothing\n");
						fflush(stdout);
				}
			}
			} else {
				printf("[UE_thread] instance_cnt_synch>=0\n");
				fflush(stdout);
                }
			...
			fflush(stdout);
                ...
                if (UE->mode != loop_through_memory) {
                    ...
                    proc->subframe_tx=sub_frame;
                    proc->frame_tx = proc->frame_rx;   
                   // proc->frame_tx = proc->frame_rx + (proc->subframe_rx>5?1:0);   
                    //proc->subframe_tx=(sub_frame+4)%10;
					if(ret==0){

							printf("[UE_thread] rxtx read nothing\n");
							fflush(stdout);
					}else{
					//解析帧号和子帧号
					struct OPENAIR_TO_SRIO_DL *header=(OPENAIR_TO_SRIO_DL*)UE->common_vars.rx_buff[0];
					uint16_t len=ntohs(header->dsp_offset);
					ENBPHYADPtoPHYType1* dspheader=(ENBPHYADPtoPHYType1*)((void*)UE->common_vars.rx_buff[0]+len);
					UE->proc.proc_rxtx[0].frame_rx=ntohs(dspheader->GHead.SFN);
					UE->proc.proc_rxtx[1].frame_rx=UE->proc.proc_rxtx[0].frame_rx;
					sub_frame=ntohs(dspheader->GHead.SubframeN);
					proc->subframe_rx=sub_frame;    
					proc->subframe_tx=sub_frame;    
                    proc->frame_tx = proc->frame_rx;   
					//proc->subframe_tx=(sub_frame+4)%10;
					//proc->frame_tx = proc->frame_rx + (proc->subframe_rx>5?1:0);   
					printf("[UE_thread] subframe=%d,frame=%d\n",sub_frame,UE->proc.proc_rxtx[0].frame_rx);
					fflush(stdout);
					}
					proc->instance_cnt_rxtx++;
					...
					printf("[UE_thread]frame=%d,subframe=%d",UE->proc.proc_rxtx[0].frame_rx,sub_frame);
					if(header->rach_msg_flag!=0||header->dsp_length!=0){
						printf("send msg rach=%d\n",header->rach_msg_flag);
						fflush(stdout);
                ...
                if (UE->mode != loop_through_memory)
                   ...
                   /* AssertFatal( UE->frame_parms.samples_per_tti*10 ==
                    ...
                    UE->rfdevice.trx_read_func(&UE->rfdevice,
                                                            ...
                                                            UE->frame_parms.nb_antennas_rx);
				AssertFatal ( 0== pthread_mutex_lock(&UE->proc.mutex_synch), "");
				printf("[UE_thread]zh test1\n");
				fflush(stdout);
                instance_cnt_synch = ++UE->proc.instance_cnt_synch;
                if (instance_cnt_synch == 0) {
                    AssertFatal( 0 == pthread_cond_signal(&UE->proc.cond_synch), "");
					printf("[UE_thread]zh test2\n");
					fflush(stdout);
                } else {
                    LOG_E( PHY, "[SCHED][UE] UE sync thread busy!!\n" );
                    exit_fun("nothing to add");
...
void init_UE_threads(PHY_VARS_UE *UE) {
    ...
    //zh add init 20171011
	UE->proc.instance_cnt_synch=-1;
	printf("[init_UE_threads]zh init instance_cnt_synch\n");
	fflush(stdout);
...
int setup_ue_buffers(PHY_VARS_UE **phy_vars_ue, openair0_config_t *openair0_cfg) {
    ...
    for (CC_id=0; CC_id<MAX_NUM_CCs; CC_id++) {
		...
		//zh add init buff 20171010
		//printf("[setup_ue_buffers]zh before init1\n");
		//fflush(stdout);
        phy_vars_ue[CC_id]->common_vars.tx_buff = (int32_t**)malloc16( frame_parms->nb_antennas_tx*sizeof(int32_t*) );
        phy_vars_ue[CC_id]->common_vars.rx_buff= (int32_t**)malloc16( frame_parms->nb_antennas_rx*sizeof(int32_t*) );
        //printf("[setup_ue_buffers]zh end init1\n");
		//fflush(stdout);
