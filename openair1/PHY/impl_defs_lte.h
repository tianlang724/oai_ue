...
typedef struct ENB_DL_TYPE1_PUBLIC_C
{
	uint16_t PDSCHNum;
	uint16_t PDCCHNum;
	uint16_t NumOfUEForPHICH;
	uint16_t Zero;
	uint32_t PBCH_C_Offset;
	uint32_t PBCH_D_Offset;
	uint32_t PHICH_C_Offset;
	uint32_t PHICH_D_Offset;
	uint32_t PDCCH_C_Offset;
	uint32_t PDCCH_D_Offset;
	uint32_t PDSCH_C_Offset;
	uint32_t PDSCH_D_Offset;
}ENB_DL_TYPE1_PUBLIC_C;
...
typedef struct ENB_DL_TYPE1_PHICH_C
{
	uint16_t	RNTI;
	uint16_t zero;
	uint32_t I_lowest_index_PRB_RA; 
	uint32_t NDmrs; 
}ENB_DL_TYPE1_PHICH_C;


typedef struct ENB_DL_TYPE1_PHICH_D
{
	uint16_t	RNTI; 
	uint16_t SourcePHICH;
}ENB_DL_TYPE1_PHICH_D;


typedef struct ENB_DL_TYPE1_PDCCH_C
{
	uint16_t	RNTI; 
	uint16_t    CommonPdcchFlag; 
	uint8_t	    PdcchFormat; 
	uint8_t 	DCIFormat; 
	uint16_t	DciBitLen; 
}ENB_DL_TYPE1_PDCCH_C;
...
typedef struct ENB_DL_TYPE1_PDSCH_C
{
	...
	uint16_t	PA; 
	uint16_t	zero; 
	uint32_t	PDSCHTbSize; //5736/4264
	uint32_t	BitMap; 
}ENB_DL_TYPE1_PDSCH_C;

typedef struct ENB_DL_TYPE1_PDSCH_D
{
	uint16_t	RNTI; 
	uint16_t PdschData; 
}ENB_DL_TYPE1_PDSCH_D;
typedef struct OPENAIR_TO_SRIO_DL
{
	uint16_t msg2_offset;
	uint16_t dsp_offset;
	uint16_t dsp_length;
}OPENAIR_TO_SRIO_DL;
...
typedef struct OPENAIR_TO_SRIO_UP
{
		uint8_t rach_msg_flag; //0 no rach  1 msg1 3msg 3
		uint16_t dsp_offset;
		uint16_t dsp_length;
}OPENAIR_TO_SRIO_UP;
