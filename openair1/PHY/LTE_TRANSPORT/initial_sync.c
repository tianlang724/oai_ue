...
//zh add new initial_sync 20171012
int initial_sync(PHY_VARS_UE *ue, runmode_t mode){
  printf("[initial_sync]zh enter\n");
  fflush(stdout);
  ...
  printf("[initial_sync]dsp offset=%d,pbch_offset=%d\n",dsp_offset,len);
  printf("[initial_sync]rx NumPrbBw=%d,PhichDurtion=%d,GPhichNg=%d\n",ntohs(pbchc->NumPrbBw),ntohs(pbchc->PhichDurtion),ntohs(pbchc->GPhichNg)); 
  fflush(stdout);
  if(ntohs(pbchc->NumPrbBw)==0){
		  return -1;
  }
  ...
  //get frame_rx
  struct ENBPHYADPtoPHYType1* dspheader=(ENBPHYADPtoPHYType1*)((void*)ue->common_vars.rx_buff[0]+dsp_offset);
  ue->proc.proc_rxtx[0].frame_rx=ntohs(dspheader->GHead.SFN);
  ue->proc.proc_rxtx[0].subframe_rx=ntohs(dspheader->GHead.SubframeN);

  ue->proc.proc_rxtx[1].frame_rx=ntohs(dspheader->GHead.SFN);
  ue->proc.proc_rxtx[1].subframe_rx=ntohs(dspheader->GHead.SubframeN);

  printf("[initial_sync] frame_rx=%d,subframe=%d\n", ue->proc.proc_rxtx[0].frame_rx,ue->proc.proc_rxtx[0].subframe_rx);
  fflush(stdout);
  ...
  LOG_I(PHY,"[UE%d] N_RB_DL %d, phich_duration %d, phich_resource %s\n",ue->Mod_id,frame_parms->N_RB_DL,
				  frame_parms->phich_config_common.phich_duration,phich_resource);
  ...
  printf("[UE_thread_synch] end\n");
  fflush(stdout);
  return 0;

}
int initial_sync_old(PHY_VARS_UE *ue, runmode_t mode)
{
  printf("[initial_sync]zh enter\n");
  fflush(stdout);
