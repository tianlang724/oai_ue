                                      ...
                                      unsigned char eNB_id )
{
  ...
  printf("[generate_ue_ulsch_params_from_rar]zh enter\n");
  //  RA_HEADER_RAPID *rarh = (RA_HEADER_RAPID *)rar_pdu;
  uint8_t transmission_mode = ue->transmission_mode[eNB_id];
  
  //zh 20171226
  /*unsigned char *rar_pdu=null;
  OPENAIR_TO_SRIO_DL* header=(OPENAIR_TO_SRIO_DL*)ue->common_vars.rx_buff[0];
  uint16_t msg2_offset=ntohs(header->msg2_offset);
  if(msg2_offset>0){
		  rar_pdu=(unsigned char*)((void*)ue->common_vars.rx_buff[0]+msg2_offset);
  }else{
		printf("[generate_ue_ulsch_params_from_rar]no rar\n");
		return;
  }*/
