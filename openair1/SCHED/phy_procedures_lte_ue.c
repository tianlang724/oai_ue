...
void ue_prach_procedures_zh(PHY_VARS_UE *ue,UE_rxtx_proc_t *proc,uint8_t eNB_id,uint8_t abstraction_flag,runmode_t mode) {
  ...
  if (ue->prach_resources[eNB_id]!=NULL) {
	...
	ue->prach_resources[eNB_id]->ra_PreambleIndex = 19;	      
      ...
      ue->prach_vars[eNB_id]->amp = AMP;
         //add msg1 to tx_buff 
		 OPENAIR_TO_SRIO_UP *header=(OPENAIR_TO_SRIO_UP*)ue->common_vars.tx_buff[0];
		 header->rach_msg_flag=1;
		 header->dsp_offset=sizeof(MSG1_Context)+sizeof(OPENAIR_TO_SRIO_UP);
		 MSG1_Context *msg1=(MSG1_Context*)((void*)ue->common_vars.tx_buff[0]+sizeof(OPENAIR_TO_SRIO_UP));
		 msg1->preamble_index=ue->prach_resources[eNB_id]->ra_PreambleIndex;
		 msg1->rarnti=htons(ue->prach_resources[eNB_id]->ra_RNTI);
...
void ue_ulsch_uespec_procedures(PHY_VARS_UE *ue,UE_rxtx_proc_t *proc,uint8_t eNB_id,uint8_t abstraction_flag) {
  printf("[ue_ulsch_uespec_procedures]zh enter\n");
  int harq_pid;
  int frame_tx=proc->frame_tx;
  int subframe_tx=proc->subframe_tx;
  int Mod_id = ue->Mod_id;
  ...
  if (ue->ulsch[eNB_id]->harq_processes[harq_pid]->subframe_scheduling_flag == 1) {
    ...
    if (Msg3_flag == 1) {
      ...
      if (abstraction_flag==0) {
	// process msg3 20171219
	OPENAIR_TO_SRIO_UP *header=(OPENAIR_TO_SRIO_UP*)ue->common_vars.tx_buff[0];
	header->rach_msg_flag=3;
	header->dsp_offset=9+sizeof(OPENAIR_TO_SRIO_UP); //test  20171219
    memcpy((void*)ue->common_vars.tx_buff[0]+sizeof(OPENAIR_TO_SRIO_UP),ue->prach_resources[eNB_id]->Msg3,9*sizeof(uint8_t));
	printf("[ue_ulsch_uespec_procedures]zh copy msg3,frame=%d,subframe=%d\n",proc->frame_tx,proc->subframe_tx);
  ...
  printf("[ue_ulsch_uespec_procedures]zh ue_get_sdu,harq_pid=%d\n",harq_pid);
	...
	//zh add copy ul data 20180109
	OPENAIR_TO_SRIO_UP *header=(OPENAIR_TO_SRIO_UP*)ue->common_vars.tx_buff[0];
	if(header->dsp_offset==0){
			header->dsp_offset=sizeof(OPENAIR_TO_SRIO_UP);
	}
	header->dsp_length=input_buffer_length;
	UE_UL_TYPE1_PUSCH_D* puschd=(UE_UL_TYPE1_PUSCH_D*)((void*)ue->common_vars.tx_buff[0]+header->dsp_offset);
	puschd->RNTI=ntohs(ue->ulsch[eNB_id]->rnti);
...
void phy_procedures_UE_TX(PHY_VARS_UE *ue,UE_rxtx_proc_t *proc,uint8_t eNB_id,uint8_t abstraction_flag,runmode_t mode,relaying_type_t r_type) {
  ...
  //printf("[phy_procedures_UE_TX] ue mode=%d,prach_config=%d\n",ue->UE_mode[eNB_id],ue->frame_parms.prach_config_common.prach_Config_enabled);
  //printf("[phy_procedures_UE_TX]frame_tx=%d,subframe_tx=%d\n",frame_tx,subframe_tx);
  //fflush(stdout);
  if ((ue->UE_mode[eNB_id] == PRACH) && 
    ...
    if (is_prach_subframe(&ue->frame_parms,frame_tx,subframe_tx)) {
      printf("[phy_procedures_UE_TX] is prach\n");
	  fflush(stdout);
      ue_prach_procedures_zh(ue,proc,eNB_id,abstraction_flag,mode);
    }
  } // mode is PRACH
  else {
    ue->generate_prach=0;
  ...
  VCD_SIGNAL_DUMPER_DUMP_FUNCTION_BY_NAME(VCD_SIGNAL_DUMPER_FUNCTIONS_PHY_PROCEDURES_UE_TX, VCD_FUNCTION_OUT);
  stop_meas(&ue->phy_proc_tx);
  //zh add to send msg 20171208 
					OPENAIR_TO_SRIO_UP *header=(OPENAIR_TO_SRIO_UP*)ue->common_vars.tx_buff[0];
					if(header->rach_msg_flag!=0||header->dsp_length!=0){
						printf("[phy_procedures_UE_TX]send msg rach=%d\n",header->rach_msg_flag);
						header->dsp_offset=htons(header->dsp_offset);
						header->dsp_length=htons(header->dsp_length);
						ue->rfdevice.trx_write_func(&ue->rfdevice,
									null,
									(void**)ue->common_vars.tx_buff,
									ue->frame_parms.samples_per_tti*10,
									 ue->frame_parms.nb_antennas_tx,0);

						header->rach_msg_flag=0;
					}else{
						printf("[phy_procedures_UE_TX] nothing to send\n");
						fflush(stdout);
					}
}
...
void ue_pbch_procedures_zh(uint8_t eNB_id,PHY_VARS_UE *ue,UE_rxtx_proc_t *proc, uint8_t abstraction_flag){
	
	int frame_rx = proc->frame_rx;
	int subframe_rx = proc->subframe_rx;
	struct OPENAIR_TO_SRIO_DL *header=(OPENAIR_TO_SRIO_DL*)ue->common_vars.rx_buff[0];
	uint16_t dsp_offset=ntohs(header->dsp_offset);
	uint32_t len=dsp_offset+sizeof(struct ENBPHYADPtoPHYType1);
	printf("[ue_pbch_procedures_zh]dsp_offset=%d,pbch_offset=%d\n",dsp_offset,len);
	...
	break;
	default:
		printf("[ue_pbch_procedures_zh] no such phich_resource\n");
	}
	if (ue->mac_enabled==1) {
		mac_xface->dl_phy_sync_success(ue->Mod_id,frame_rx,eNB_id,ue->UE_mode[eNB_id]==NOT_SYNCHED ? 1 : 0);
	}
		
}
//20171120 zh add
int ue_pdcch_procedures_zh(uint8_t eNB_id,PHY_VARS_UE *ue,UE_rxtx_proc_t *proc,uint8_t abstraction_flag)
{
  //daiwaishan
	...
	struct OPENAIR_TO_SRIO_DL *header=(OPENAIR_TO_SRIO_DL*)ue->common_vars.rx_buff[0];
	struct ENBPHYADPtoPHYType1 *dspheader=(ENBPHYADPtoPHYType1*)((void*)ue->common_vars.rx_buff[0]+ntohs(header->dsp_offset));
	int num_dci=ntohs(dspheader->DL_TYPE1_PUBLIC_C.PDCCHNum);
	int public_offset=ntohs(header->dsp_offset)+sizeof(ENBPHYADPtoPHYType1)+sizeof(ENB_DL_TYPE1_PBCH_C)+sizeof(ENB_DL_TYPE1_PBCH_D)+2*sizeof(ENB_DL_TYPE1_PHICH_C)+2*sizeof(ENB_DL_TYPE1_PHICH_D);
    ENB_DL_TYPE1_PDCCH_C *pdcchc=(ENB_DL_TYPE1_PDCCH_C*)((void*)ue->common_vars.rx_buff[0]+public_offset);
	ENB_DL_TYPE1_PDCCH_D *pdcchd=(ENB_DL_TYPE1_PDCCH_D*)((void*)ue->common_vars.rx_buff[0]+public_offset+sizeof(ENB_DL_TYPE1_PDCCH_C)*5);
	/*
	ENB_DL_TYPE1_PDCCH_C *tpdcchc;
	ENB_DL_TYPE1_PDCCH_D *tpdcchd;
	...
	for(i=0;i<num_dci;i++){
	    	...
	    	uint8_t dci_pdu[8];
			memcpy(dci_pdu,&pdcchd[i].DataSource,sizeof(pdcchd[i].DataSource));
		if ((ue->UE_mode[eNB_id]>PRACH) &&
	    	...
	    	//uint8_t dci_pdu[8];
			//memcpy(dci_pdu,&pdcchd[i].DataSource,sizeof(pdcchd[i].DataSource));
			//printf("[pdcchd] dci_alloc=%x %x %x %x %x %x %x %x\n",dci_pdu[0],dci_pdu[1],dci_pdu[2],dci_pdu[3],dci_pdu[4],dci_pdu[5],dci_pdu[6],dci_pdu[7]);
      			if ((ue->UE_mode[eNB_id] > PRACH) &&
	 					 (generate_ue_dlsch_params_from_dci(frame_rx,
					     subframe_rx,
					     dci_pdu,
					     ue->pdcch_vars[subframe_rx & 0x1][eNB_id]->crnti,
					     pdcchc[i].PdcchFormat,
          ...
          if((pdcchc[i].PdcchFormat== format1)   ||
              (pdcchc[i].PdcchFormat == format1A) ||
              (pdcchc[i].PdcchFormat == format1B) ||
              (pdcchc[i].PdcchFormat == format2)  ||
              (pdcchc[i].PdcchFormat == format2A) ||
              (pdcchc[i].PdcchFormat == format2B))
          {
	...
	}else if((ntohs(pdcchc[i].RNTI)==SI_RNTI)&&((pdcchc[i].PdcchFormat==format1A)||(pdcchc[i].PdcchFormat==format1C))){
			/*
			printf("[ue_pdcch_procedures_zh]si pdcchc offset=%d,si pdcchd offset=%d\n",public_offset,public_offset+sizeof(ENB_DL_TYPE1_PDCCH_C)*5);
			printf("pdcchc rnti=%d,flag=%d,format=%d,bitlen=%d\n",ntohs(pdcchc[i].RNTI),ntohs(pdcchc[i].CommonPdcchFlag),pdcchc[i].PdcchFormat,ntohs(pdcchc[i].DciBitLen));
			printf("pdcchd rnti=%d,datasource=%llx\n",ntohs(pdcchd[i].RNTI),pdcchd[i].DataSource);
			fflush(stdout);*/
	    	//uint8_t dci_pdu[8];
			//memcpy(dci_pdu,&pdcchd[i].DataSource,sizeof(pdcchd[i].DataSource));
			//printf("[pdcchd] dci_alloc=%x %x %x %x %x %x %x %x\n",dci_pdu[0],dci_pdu[1],dci_pdu[2],dci_pdu[3],dci_pdu[4],dci_pdu[5],dci_pdu[6],dci_pdu[7]);
				if(generate_ue_dlsch_params_from_dci(frame_rx,
					...
					printf("[ue_pdcch_procedures_zh] zh generate_ue_dlsch_params_from_dci si_rnti ok\n");
					fflush(stdout);
		}else{
				printf("[ue_pdcch_procedures_zh] zh generate_ue_dlsch_params_from_dci si_rnti failed\n");
				fflush(stdout);
		}
	}else if ((ntohs(pdcchc[i].RNTI)  == P_RNTI) &&
	     ((pdcchc[i].PdcchFormat== format1A) || (pdcchc[i].PdcchFormat == format1C))) {

				 printf("[ue_pdcch_procedures_zh] zh p_rnti\n");
	}else if((ue->prach_resources[eNB_id]) 
					&&(ntohs(pdcchc[i].RNTI) == ue->prach_resources[eNB_id]->ra_RNTI) 
					&&(pdcchc[i].PdcchFormat == format1A)){
			// process ra dci
			printf("[ue_pdcch_procedures_zh] ra dci\n");
	    	//uint8_t dci_pdu[8];
			//memcpy(dci_pdu,&pdcchd[i].DataSource,sizeof(pdcchd[i].DataSource));
			//printf("[pdcchd] dci_alloc=%x %x %x %x %x %x %x %x\n",dci_pdu[0],dci_pdu[1],dci_pdu[2],dci_pdu[3],dci_pdu[4],dci_pdu[5],dci_pdu[6],dci_pdu[7]);
			      if (generate_ue_dlsch_params_from_dci(frame_rx,
										  subframe_rx,
										  (DCI1A_5MHz_TDD_1_6_t *)&dci_pdu,
										  ue->prach_resources[eNB_id]->ra_RNTI,
										  format1A,
										  &ue->dlsch_ra[eNB_id],
										  &ue->frame_parms,
										  ...
										  ue->prach_resources[eNB_id]->ra_RNTI,
										  P_RNTI,
										  ue->transmission_mode[eNB_id]<7?0:ue->transmission_mode[eNB_id],0)==0){
				 ...
				 printf("[ue_pdcch_procedures_zh] ra crnti\n");
				       if ((ue->UE_mode[eNB_id] > PRACH) &&
							(generate_ue_ulsch_params_from_dci((void *)&dci_pdu,
							              ...
							              CBA_RNTI,
									      eNB_id,0)==0)){
							   LOG_I(PHY,"[UE  %d] Generate UE ULSCH C_RNTI format 0 (subframe %d)\n",ue->Mod_id,subframe_rx);
					   }
	}else if( (ntohs(pdcchc[i].RNTI)== ue->ulsch[eNB_id]->cba_rnti[0]) &&
						       (pdcchc[i].PdcchFormat == format0)){
				 printf("[ue_pdcch_procedures_zh] zh cba crnti,rnti=%d,do nothing\n",ue->ulsch[eNB_id]->cba_rnti[0]);
				 /*
				 ue->ulsch_no_allocation_counter[eNB_id] = 0;
				       if ((ue->UE_mode[eNB_id] > PRACH) &&
									 (generate_ue_ulsch_params_from_dci((void *)&dci_pdu,
											...
											eNB_id,
									        0)==0)) {
							   LOG_I(PHY,"[UE  %d] Generate UE ULSCH CBA_RNTI format 0 (subframe %d)\n",ue->Mod_id,subframe_rx);
							   ue->ulsch[eNB_id]->num_cba_dci[(subframe_rx+4)%10]++;
					    }*/ 
	}else{
			//待完善
			printf("[ue_pdcch_procedures_zh] no such pdcchc.rnti=%d\n",ntohs(pdcchc[i].RNTI));
			fflush(stdout);
	}
...
void ue_dlsch_procedures_zh(PHY_VARS_UE *ue,UE_rxtx_proc_t *proc,int eNB_id,PDSCH_t pdsch, runmode_t mode, int abstraction_flag,uint16_t pdschd_offset) {
 ...
 /*	OPENAIR_TO_SRIO_DL *header=(OPENAIR_TO_SRIO_DL*)ue->common_vars.rx_buff[0];
	...
	int dsp_offset=ntohs(header->dsp_offset);
	int pdschd_offset=dsp_offset+sizeof(ENBPHYADPtoPHYType1)+
			sizeof(ENB_DL_TYPE1_PBCH_C)+
			sizeof(ENB_DL_TYPE1_PBCH_D)+
			2*sizeof(ENB_DL_TYPE1_PHICH_C)+
			2*sizeof(ENB_DL_TYPE1_PHICH_D)+
			5*sizeof(ENB_DL_TYPE1_PDCCH_C)+
		...
		case PDSCH:
			...
			/*	for(int i=0;i<len;i++){
				...
				mac_xface->ue_send_sdu(ue->Mod_id,
								...
								frame_rx,
								subframe_rx,
								(void*)pdschd+sizeof(ENB_DL_TYPE1_PDSCH_D),
								len,
								eNB_id);
		...
		case SI_PDSCH:	
				...
				// }
				 mac_xface->ue_decode_si(ue->Mod_id,
							    ...
							    frame_rx,
						        eNB_id,
					            (void*)ue->common_vars.rx_buff[0]+pdschd_offset+sizeof(ENB_DL_TYPE1_PDSCH_D),
								len);
				 }
				break;
		case P_PDSCH:
				printf("[ue_dlsch_procedures_zh] p_psdch\n");
				break;
		case RA_PDSCH:
				printf("[ue_dlsch_procedures_zh] ra \n");
				fflush(stdout);
				process_rar_zh(ue,proc,eNB_id,mode,abstraction_flag);
				break;
		case PDSCH1:
				printf("[ue_dlsch_procedures_zh] pdsch1 \n");
				break;
		case PMCH:
				printf("[ue_dlsch_procedures] no PMCH\n");
				break;
...
void process_rar_zh(PHY_VARS_UE *ue, UE_rxtx_proc_t *proc, int eNB_id, runmode_t mode, int abstraction_flag) {
  

  
	struct OPENAIR_TO_SRIO_DL *header=(OPENAIR_TO_SRIO_DL*)ue->common_vars.rx_buff[0];
	struct ENBPHYADPtoPHYType1 *dspheader=(ENBPHYADPtoPHYType1*)((void*)ue->common_vars.rx_buff[0]+ntohs(header->dsp_offset));
	uint16_t rar_offset=ntohs(header->msg2_offset);
	uint16_t rar_len=ntohs(header->dsp_offset)-rar_offset;
	uint8_t *rar_data=(uint8_t*)((void*)ue->common_vars.rx_buff[0]+rar_offset);
	
	printf("[process_rar_zh] rar_offset=%d rar_len=%d\n",rar_offset,rar_len);
	fflush(stdout);
	/*
	for(int i=0;i<7;i++){
			printf(" %d:%x ",i,rar_data[i]);
	}*/
  ...
  if (ue->mac_enabled == 1) {
	...
	(ue->prach_resources[eNB_id]->Msg3!=NULL)) {
      ...
      timing_advance = mac_xface->ue_process_rar(ue->Mod_id,
						 ...
						 &ue->pdcch_vars[subframe_rx & 0x1][eNB_id]->crnti,
						 ue->prach_resources[eNB_id]->ra_PreambleIndex,
						 rar_data); // alter the 'b' buffer so it contains only the selected RAR header and RAR payload

	  memcpy(dlsch0->harq_processes[0]->b,rar_data,rar_len); // alter the 'b' buffer so it contains only the selected RAR header and RAR payload
...
int phy_procedures_UE_RX_zh(PHY_VARS_UE *ue,UE_rxtx_proc_t *proc,uint8_t eNB_id,uint8_t abstraction_flag,runmode_t mode,
		           relaying_type_t r_type,PHY_VARS_RN *phy_vars_rn) {
  printf("[phy_procesures_UE_RX_zh] enter\n");
  fflush(stdout);
  int frame_rx = proc->frame_rx;
  int subframe_rx = proc->subframe_rx;
  ...
  ue_pdcch_procedures_zh(eNB_id,ue,proc,abstraction_flag);
	
   if(subframe_rx == 0){
	 ue_pbch_procedures_zh(eNB_id,ue,proc,abstraction_flag);
   }
   
 if ((ue->dlsch_ra[eNB_id]) && (ue->dlsch_ra[eNB_id]->active == 1)) {
	 printf("[phy_procedures_UE_RX]zh ue_dlsch_procedures_zh will process ra\n");
	 fflush(stdout);
...
int phy_procedures_UE_RX(PHY_VARS_UE *ue,UE_rxtx_proc_t *proc,uint8_t eNB_id,uint8_t abstraction_flag,runmode_t mode,
  ...
  int frame_rx = proc->frame_rx;
  int subframe_rx = proc->subframe_rx;
  printf("[phy_procesures_UE_RX] enter\n");
