...
int decode_BCCH_DLSCH_Message(
  ...
  const uint8_t                rsrp )
{
  printf("[decode_BCCH_DLSCH_Message] zh enter\n");
  fflush(stdout);
...
# endif
  ...
  printf("[decode_BCCH_DLSCH_Message] bcch_message->message.present=%d,bcch_message->message.choice.c1.present=%d\n",bcch_message->message.present,bcch_message->message.choice.c1.present);
  printf("[decode_BCCH_DLSCH_Message] BCCH_DL_SCH_MessageType_PR_c1=%d,BCCH_DL_SCH_MessageType__c1_PR_systemInformationBlockType1=%d\n",BCCH_DL_SCH_MessageType_PR_c1,BCCH_DL_SCH_MessageType__c1_PR_systemInformationBlockType1);
  fflush(stdout);
  if (bcch_message->message.present == BCCH_DL_SCH_MessageType_PR_c1) {
    switch (bcch_message->message.choice.c1.present) {
    case BCCH_DL_SCH_MessageType__c1_PR_systemInformationBlockType1:
	  printf("case BCCH_DL_SCH_MessageType__c1_PR_systemInformationBlockType1\n");
      if ((ctxt_pP->frame % 2) == 0) {
        ...
        if ((UE_rrc_inst[ctxt_pP->module_id].Info[eNB_index].SIStatus&1) == 0) {
          ...
          printf("[decode_BCCH_DLSCH_Message] case BCCH_DL_SCH_MessageType__c1_PR_systemInformationBlockType1 depcode SIB1\n");
		  fflush(stdout);
    ...
    case BCCH_DL_SCH_MessageType__c1_PR_systemInformation:
	  printf("case BCCH_DL_SCH_MessageType__c1_PR_systemInformation\n");
      if ((UE_rrc_inst[ctxt_pP->module_id].Info[eNB_index].SIStatus&1) == 1) {
        ...
        LOG_D( RRC, "[UE %"PRIu8"] Decoding SI for frameP %"PRIu32"\n",
               ctxt_pP->module_id,
               ctxt_pP->frame );
		printf("[decode_BCCH_DLSCH_Message] case BCCH_DL_SCH_MessageType__c1_PR_systemInformation depcode SI\n");
		fflush(stdout);
...
static int decode_SIB1( const protocol_ctxt_t* const ctxt_pP, const uint8_t eNB_index, const uint8_t rsrq, const uint8_t rsrp )
{
  ...
  printf("[decode_SIB1]zh will call rrc_mac_config_req\n");
  fflush(stdout);
  rrc_mac_config_req(ctxt_pP->module_id, 0, ENB_FLAG_NO, 0, eNB_index,
                      (RadioResourceConfigCommonSIB_t *)NULL,
                      (struct PhysicalConfigDedicated *)NULL,
  ...
  printf("[decode_SIB1] zh set SIStatues=1\n");
  fflush(stdout);
...
static int decode_SI( const protocol_ctxt_t* const ctxt_pP, const uint8_t eNB_index )
{
  ...
  for (int i=0; i<(*si)->criticalExtensions.choice.systemInformation_r8.sib_TypeAndInfo.list.count; i++) {
	...
	LOG_I( RRC, "[FRAME %05"PRIu32"][RRC_UE][MOD %02"PRIu8"][][--- MAC_CONFIG_REQ (SIB2 params  eNB %"PRIu8") --->][MAC_UE][MOD %02"PRIu8"][]\n",
	       ctxt_pP->frame, ctxt_pP->module_id, eNB_index, ctxt_pP->module_id );
	printf("[decode_SI] switch SystemInformation_r8_IEs__sib_TypeAndInfo__Member_PR_sib2\n");
	fflush(stdout);
	rrc_mac_config_req(ctxt_pP->module_id, 0, ENB_FLAG_NO, 0, eNB_index,
...
# if defined(ENABLE_USE_MME)
    ...
    case NAS_CELL_SELECTION_REQ:
      printf("[rrc_ue_task] get NAS_CELL_SELECTION_REQ");
	  fflush(stdout);
      ...
      if (rrc_get_state(ue_mod_id) == RRC_STATE_INACTIVE) {
        printf("[rrc_ue_task]have a look at MAC/main.c void dl_phy_sync_success(...)\n");
		fflush(stdout);
      ...
      case RRC_STATE_INACTIVE: {
	    ...
	    printf("[rrc_ue_task] case RRC_STATE_INACTIVE will set to IDLE\n");
		fflush(stdout);
